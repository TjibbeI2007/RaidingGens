...
public class EntranceWestModel extends Model {
    public EntranceWestModel(Location location, int priority, int placeChance) {
        super(location,
    ...
    public HashMap<Location, Material> getBlockLocations() {
        ...
        return new HashMap<>(ModelUtils.getLocations(
                this.corners.get("maxX_maxY_maxZ").clone().add(0, -margin, -margin),
                this.corners.get("maxX_minY_minZ").clone().add(0, margin, margin),
                Material.AIR,
                ModelMode.YZ
        ));
    }
