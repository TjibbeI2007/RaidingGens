...
public class EntranceSouthModel extends Model {
    public EntranceSouthModel(Location location, int priority, int placeChance) {
        super(location,
    ...
    public HashMap<Location, Material> getBlockLocations() {
        ...
        return ModelUtils.getLocations(
                this.corners.get("minX_minY_minZ").clone().add(margin,margin,0),
                this.corners.get("maxX_maxY_minZ").clone().add(-margin,-margin,0),
                Material.AIR,
                ModelMode.XY
        );
    }
